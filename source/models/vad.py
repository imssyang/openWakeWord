...
class SileroVadModel():
    def __init__(
        ...
        *,
        ...
        self.sample_rates = self._supported_sample_rates(onnx_path)
        self.session = self._create_session(onnx_path, n_threads, provider)
        self.reset_states()

    def _create_session(self, onnx_path: str, n_threads: int, provider: str):
        opts = ort.SessionOptions()
        opts.inter_op_num_threads = n_threads
        opts.intra_op_num_threads = n_threads
        if provider == "cpu" and 'CPUExecutionProvider' in ort.get_available_providers():
    ...
    def _supported_sample_rates(self, onnx_path: str):
        if '16k' in onnx_path:
    ...
    def _validate_input(self, x: np.ndarray, sr: int):
        if x.ndim == 1:
            x = x.unsqueeze(0) # shape to [ B, T ], B=batch=1, T=num_samples=512
        if x.ndim > 2:
